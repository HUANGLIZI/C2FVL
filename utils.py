...
class WeightedDiceLoss(nn.Module):
    ...
    def forward(self, logit, truth, smooth=1e-5):
        ...
        #print('dice',dice)
        loss = dice.mean()
        #print('dice.mean',loss)
        return loss
class Cosineloss(nn.Module):
    def __init__(self):
        super(Cosineloss, self).__init__()
    def forward(self, py1,py2,py3,py4):
        batch_size = len(py1)
        p4=py4.view(batch_size,-1)
        p1=py1.view(batch_size,-1)
        p2=py2.view(batch_size,-1)
        p3=py3.view(batch_size,-1)
        #loss=[]
        #for i in range(batch_size):
          ...
          #p2=py2[i][0].ravel()
          #p3=py2[i][0].ravel()
        l12=1-torch.cosine_similarity(p1, p2, dim=1)
        l13=1-torch.cosine_similarity(p1, p3, dim=1)
        l14=1-torch.cosine_similarity(p1, p4, dim=1)
          #l24=1-(p4*p2).sum(-1) / (linalg.norm(p4.cpu().numpy()) * linalg.norm(p2.cpu().numpy()))
          #l34=1-(p4*p3).sum(-1) / (linalg.norm(p4.cpu().numpy()) * linalg.norm(p3.cpu().numpy()))
        loss=0.5*l12+0.5*l13+0.5*l14
        loss=loss.mean()
          #print(type(l))
          #loss.append(l)
        #print(type(loss))
...
class WeightedDiceBCE(nn.Module):
    ...
    def forward(self, inputs, targets,py1,py2,py3,py4):
        dice = self.dice_loss(inputs, targets)
        BCE = self.BCE_loss(inputs, targets)
        dice_BCE_loss = self.dice_weight * dice + self.BCE_weight * BCE
        #print('dice_BCE_loss',dice_BCE_loss)
        #print('dice_BCE_loss.shape:',dice_BCE_loss.shape)
        cosineloss=self.cosloss(py1,py2,py3,py4)     
        return dice_BCE_loss+cosineloss
...
def read_attr(filename):
    ...
    rl={}
    for i in df.index.values:  # Gets the index of the row number and traverses it
        bt[df.Image[i]]= df.bilateral[i]
        num[df.Image[i]] = df.number[i]
        lu[df.Image[i]]= df.lupper[i]
        lm[df.Image[i]] = df.lmiddle[i]
        ll[df.Image[i]]= df.llower[i]
        ru[df.Image[i]] = df.rupper[i]
        rm[df.Image[i]]= df.rmiddle[i]
        rl[df.Image[i]] = df.rlower[i]
    return bt,num,lu,lm,ll,ru,rm,rl  # return dict (key: values)
...
def get_thum(image, size=(28, 28), greyscale=False):
