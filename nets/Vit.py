...
class Reconstruct_text(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, scale_factor):
        super(Reconstruct_text, self).__init__()
        if kernel_size == 3:
            padding = 1
        else:
            padding = 0
    ...
    def forward(self, x):
        ...
        B, hidden, n_patch = x.size()  # reshape from (B, hidden, n_patch) to (B, hidden, h, w)
        h, w = int(n_patch/2), int(n_patch/2)
        #x = x.permute(0, 2, 1)
        x = x.contiguous().view(B, hidden, 2, 4)
        x = nn.Upsample(scale_factor=self.scale_factor)(x)
...
class Process_text(nn.Module):
    ...
    def forward(self, x,size):
        if x is None:
            return None
        #print(x.size())
        B, n_patch,hidden = x.size()  # reshape from (B, hidden, n_patch) to (B, hidden, h, w)
        #print(size,hidden)
        x = x.contiguous().view(B, hidden)
        x = x.repeat(1, int(size/hidden))
        #print(x.shape)
